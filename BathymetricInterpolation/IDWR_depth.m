...
% % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % %
% Function: Interpolate water depth data on a rectangular grid to query points using the Inverse Distance Weighted Regression (IDWR) method.
% Inputs:
...
for j = 1:M  % Loop through all unknown points
    ...
    % Call the IDWR function to calculate the value at the unknown point (X(j), Y(j))
    K = numel(Z1);
    v = IDWR(X1(:), Y1(:), Z1(:), [X(j)], [Y(j)], K);  % Note: The query point vector contains only one point (X(j), Y(j))
    V(j) = v(1);
end
end
