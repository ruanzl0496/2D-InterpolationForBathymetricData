...
% % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % %
% 矩形网格水深数据插值，反距离加权（Inverse Distance Weighted,IDW）算法
% 输入：
%   x,y,z--均匀矩形网格点对应的数据向量，一行中x坐标是增加的，y坐标相同，但是第i行的y坐标大于第i+1行的y坐标
%   X,Y--为未知点坐标向量,或矩阵
% 输出：V--未知点的值向量或矩阵
% % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % %
%%
...
for j = 1:M  % 循环所有未知点
    ...
    K = numel(Z1);
    v = IDW(X1(:),Y1(:),Z1(:),[X(j)],[Y(j)],K);  % 注意，这里查询点向量进含一个点（X(j),Y(j)）
    V(j) = v(1);
end
end
